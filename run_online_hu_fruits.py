...
CSV_PATH = os.path.join("features_out", "X_hu_fruits.csv")
...
def main():
    ...
    # Restricción de tamaño usando el ground truth (conteo por clase)
    counts = classes.value_counts().sort_index().values
    max_sizes = counts.tolist()

    model = OnlineConstrainedKMeans(n_clusters=k, max_sizes=max_sizes, random_state=42)
    pred = model.partial_fit_predict(Xs)

    print("=== ONLINE CONSTRAINED KMEANS | HU FRUITS ===")
    print("k:", k)
