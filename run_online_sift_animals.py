...
CSV_PATH = os.path.join("features_out", "X_sift_animals.csv")

def main():
    df = pd.read_csv(CSV_PATH)

    sift_cols = [c for c in df.columns if c.startswith("sift_")]
    X = df[sift_cols].values.astype(np.float32)
    ...
    print("=== ONLINE CONSTRAINED KMEANS | SIFT ANIMALS (PCA=50) ===")
    print("k:", k)
